...
trait ValidatesRetailSaleCartLines
{
    /**
     * @return array<string, mixed>
     */
    protected function retailCartLinesRules(int $branchId): array
    {
        return [
            'warehouse_id' => [
                'required',
                'integer',
                \Illuminate\Validation\Rule::exists('warehouses', 'id')->where(fn ($q) => $q->where('branch_id', $branchId)),
            ],
    ...
    protected function validateRetailCartLinesAfter(Validator $v, ?int $forceWarehouseId = null): void
    {
        ...
        $warehouseId = $forceWarehouseId !== null ? $forceWarehouseId : (int) $this->input('warehouse_id');
        $hasAny = false;
        $articleCounts = [];
