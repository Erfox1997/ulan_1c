    ...
    <div class="mx-auto max-w-2xl space-y-6">
        ...
        <div class="overflow-hidden rounded-xl border border-slate-200/90 bg-white shadow-md ring-1 ring-slate-900/[0.04]">
            ...
            >
                ...
                <p class="mt-0.5 text-[11px] font-medium text-emerald-100/90">{{ $advance->employee->full_name }}</p>
            </div>

            @include('admin.payroll.partials.employee-amount-form', [
                'action' => route('admin.payroll.advances.update', $advance),
                'method' => 'PUT',
                'employees' => $employees,
                'submitLabel' => 'Сохранить',
                'cancelRoute' => 'admin.payroll.advances.index',
                'selectedEmployeeId' => old('employee_id', $advance->employee_id),
                'entryDate' => old('entry_date', $advance->entry_date?->format('Y-m-d')),
                'amount' => old('amount', $advance->amount),
                'note' => old('note', $advance->note),
            ])
        </div>
    </div>
</x-admin-layout>
