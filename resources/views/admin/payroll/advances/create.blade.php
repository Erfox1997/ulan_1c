<x-admin-layout :pageTitle="$pageTitle" main-class="bg-slate-100/80 px-3 py-4 sm:px-4 lg:px-6">
    <div class="mx-auto max-w-2xl space-y-6">
        ...
        @if ($employees->isEmpty())
            ...
            <p class="text-sm"><a href="{{ route('admin.payroll.advances.index') }}" class="text-emerald-700 hover:text-emerald-900">← Назад к списку</a></p>
        @else
            <div class="overflow-hidden rounded-xl border border-slate-200/90 bg-white shadow-md ring-1 ring-slate-900/[0.04]">
                ...
                >
                    ...
                    <p class="mt-0.5 text-[11px] font-medium text-emerald-100/90">Сотрудник выбирается из справочника филиала.</p>
                </div>

                @include('admin.payroll.partials.employee-amount-form', [
                    'action' => route('admin.payroll.advances.store'),
                    'method' => 'POST',
                    'employees' => $employees,
