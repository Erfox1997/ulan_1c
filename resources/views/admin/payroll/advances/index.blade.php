    ...
    <div class="mx-auto max-w-6xl space-y-6">
        ...
        <div class="flex flex-wrap items-center justify-between gap-3">
            <div>
                ...
                <p class="mt-0.5 text-sm text-slate-600">Выплаты аванса сотрудникам филиала (учитываются при расчёте зарплаты).</p>
            </div>
            @if ($employees->isNotEmpty())
                <a
                    href="{{ route('admin.payroll.advances.create') }}"
                        ...
                        @forelse ($advances as $row)
                            <tr class="hover:bg-emerald-50/30">
                                ...
                                <td class="px-4 py-2.5 text-right whitespace-nowrap">
                                    <a href="{{ route('admin.payroll.advances.edit', $row) }}" class="text-sm font-medium text-emerald-700 hover:text-emerald-900">Изменить</a>
                                    <form
                                        method="POST"
                                        action="{{ route('admin.payroll.advances.destroy', $row) }}"
                                        class="ml-3 inline"
