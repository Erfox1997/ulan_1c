@php
    ...
    $bankMovementCpListFilterInit = [
        'searchUrl' => route('admin.counterparties.search', ['for' => 'purchase']),
        'listUrl' => route('admin.bank.expense-supplier'),
        'initialValue' => $filterInputValue,
        'appliedCounterpartyId' => $cpIdFilter,
        'allowedKinds' => ['supplier', 'other'],
    ];
@endphp
<x-admin-layout pageTitle="Расход: оплата поставщику" main-class="bg-slate-100/80 px-3 py-5 sm:px-6 lg:px-8">
    <style>
        .income-client-cp-dd {
            max-height: 16rem;
            overflow-y: auto;
    ...
    <div class="mx-auto w-full max-w-5xl space-y-4">
        ...
        <div class="flex flex-col gap-4 sm:flex-row sm:flex-wrap sm:items-center sm:justify-between">
            <div class="min-w-0">
                ...
                <p class="mt-0.5 text-sm text-slate-600">
                    Операции расхода на оплату поставщикам по филиалу (последние 500 записей).
                    @if ($hasSearch)
                        @if ($cpIdFilter !== null)
            ...
            <a
                href="{{ route('admin.bank.expense-supplier.create') }}"
                class="inline-flex w-full shrink-0 items-center justify-center rounded-xl border border-teal-900/20 bg-teal-600 px-5 py-3 text-center text-sm font-bold !text-white no-underline shadow-[0_10px_28px_-6px_rgba(13,148,136,0.55)] transition hover:bg-teal-500 active:bg-teal-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-teal-500 focus-visible:ring-offset-2 sm:w-auto visited:!text-white"
            >
                + Создать операцию
            </a>
        </div>
        ...
        <form
            method="get"
            action="{{ route('admin.bank.expense-supplier') }}"
        ...
        >
            <div class="relative min-w-0 flex-1" x-ref="bankCpListFilterRoot">
                <label for="expense_supplier_q" class="mb-1 block text-xs font-semibold text-slate-600">Поставщик (контрагент)</label>
                <input
                    id="expense_supplier_q"
                    type="search"
                    autocomplete="off"
                    x-model="query"
                    ...
                    x-on:blur="onCpBlur()"
                    class="min-h-[2.75rem] w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm text-slate-900 shadow-inner shadow-slate-900/[0.03] placeholder:text-slate-400 focus:border-teal-500 focus:outline-none focus:ring-2 focus:ring-teal-500/25"
            ...
            <div class="mt-2 flex shrink-0 flex-wrap gap-2 sm:mt-7">
                <button
                    type="submit"
                    class="inline-flex min-h-[2.75rem] items-center justify-center rounded-xl border border-teal-800/25 bg-teal-700 px-4 py-2 text-sm font-semibold text-white shadow-sm transition hover:bg-teal-600 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-teal-500 focus-visible:ring-offset-2"
                >
                    Найти по тексту
                </button>
                @if ($hasSearch)
        ...
        <div class="overflow-hidden rounded-2xl border border-slate-200/90 bg-white shadow-md ring-1 ring-slate-900/[0.04]">
            <div
                class="border-b border-teal-900/10 px-4 py-3 text-white sm:px-5"
                style="background: linear-gradient(120deg, #0d9488 0%, #059669 45%, #047857 100%);"
            >
                ...
                <table class="min-w-full text-left text-sm">
                    <thead>
                        <tr class="border-b border-slate-200 bg-slate-50/95 text-[10px] font-bold uppercase tracking-wide text-slate-500">
                            ...
                            <th class="min-w-[12rem] px-4 py-2.5">Поставщик</th>
                    ...
                    <tbody class="divide-y divide-slate-100">
                        @forelse ($movements as $m)
                            ...
                            @endphp
                            <tr
                                class="cursor-pointer transition-colors hover:bg-teal-50/70 focus-visible:outline focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-teal-400"
                                role="link"
                                tabindex="0"
                                title="Открыть для редактирования"
                                onclick="window.location.href={{ json_encode(route('admin.bank.expense-supplier.edit', $m->id)) }}"
                                onkeydown="if(event.key==='Enter'||event.key===' '){event.preventDefault();window.location.href={{ json_encode(route('admin.bank.expense-supplier.edit', $m->id)) }};}"
                            >
                                <td class="whitespace-nowrap px-4 py-3 tabular-nums text-slate-800">{{ $m->occurred_on?->format('d.m.Y') ?? '—' }}</td>
                                <td class="whitespace-nowrap px-4 py-3 text-right font-semibold tabular-nums text-red-800">{{ $fmtMoney($m->amount) }}</td>
                                ...
                                <td colspan="6" class="px-4 py-12 text-center text-sm text-slate-500">
                                    @if ($hasSearch)
                                        @if ($cpIdFilter !== null)
                                            ...
                                            <a href="{{ route('admin.bank.expense-supplier') }}" class="font-semibold text-teal-800 underline decoration-teal-400 underline-offset-2 hover:text-teal-950">Показать все</a>
                                        @else
                                            По запросу «{{ $searchQuery }}» операций не найдено.
                                            <a href="{{ route('admin.bank.expense-supplier') }}" class="font-semibold text-teal-800 underline decoration-teal-400 underline-offset-2 hover:text-teal-950">Показать все</a>
                                        @endif
                                    @else
                                        Пока нет операций. Нажмите «Создать операцию», чтобы записать оплату поставщику.
                                    @endif
                                </td>
                            </tr>
                        @endforelse
