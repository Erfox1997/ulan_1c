...
@endphp
<x-admin-layout pageTitle="Расход: прочие" main-class="bg-slate-100/80 px-3 py-5 sm:px-6 lg:px-8">
    <div class="mx-auto w-full max-w-5xl space-y-4">
        ...
        <div class="flex flex-col gap-4 sm:flex-row sm:flex-wrap sm:items-center sm:justify-between">
            <div class="min-w-0">
                ...
                <p class="mt-0.5 text-sm text-slate-600">Прочие расходы по филиалу (последние 500 записей).</p>
            </div>
            <a
                href="{{ route('admin.bank.expense-other.create') }}"
                class="inline-flex w-full shrink-0 items-center justify-center rounded-xl border border-indigo-900/15 bg-indigo-600 px-5 py-3 text-center text-sm font-bold !text-white no-underline shadow-[0_10px_28px_-6px_rgba(79,70,229,0.5)] transition hover:bg-indigo-500 active:bg-indigo-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500 focus-visible:ring-offset-2 sm:w-auto visited:!text-white"
            >
                + Создать операцию
            </a>
        </div>
        ...
        <div class="overflow-hidden rounded-2xl border border-slate-200/90 bg-white shadow-md ring-1 ring-slate-900/[0.04]">
            <div
                class="border-b border-indigo-900/10 px-4 py-3 text-white sm:px-5"
                style="background: linear-gradient(120deg, #4f46e5 0%, #6366f1 45%, #4338ca 100%);"
            >
                ...
                <table class="min-w-full text-left text-sm">
                    <thead>
                        <tr class="border-b border-slate-200 bg-slate-50/95 text-[10px] font-bold uppercase tracking-wide text-slate-500">
                            ...
                            <th class="min-w-[10rem] px-4 py-2.5">Категория</th>
                    ...
                    <tbody class="divide-y divide-slate-100">
                        @forelse ($movements as $m)
                            <tr
                                class="cursor-pointer transition-colors hover:bg-indigo-50/70 focus-visible:outline focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-indigo-400"
                                role="link"
                                tabindex="0"
                                title="Открыть для редактирования"
                                onclick="window.location.href={{ json_encode(route('admin.bank.expense-other.edit', $m->id)) }}"
                                onkeydown="if(event.key==='Enter'||event.key===' '){event.preventDefault();window.location.href={{ json_encode(route('admin.bank.expense-other.edit', $m->id)) }};}"
                            >
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="px-4 py-12 text-center text-sm text-slate-500">
                                    Пока нет операций. Нажмите «Создать операцию», чтобы записать прочий расход.
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
