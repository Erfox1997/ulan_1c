<x-admin-layout pageTitle="Редактирование поступления" main-class="px-3 py-5 sm:px-5 lg:px-7 bg-[#f0f4f8]">
    ...
    <div class="cp-root mx-auto w-full max-w-[min(100%,112rem)] space-y-6">
        ...
        <div class="flex flex-wrap items-center gap-x-4 gap-y-2 text-sm">
            <a
                ...
                class="font-semibold text-sky-800 decoration-sky-300 underline-offset-2 hover:text-sky-950 hover:underline"
        ...
        @else
            ...
            @include('admin.purchase-receipts.partials.create-page-styles')
            ...
            @endphp
                ...
                <script>
                    window.__purchaseReceiptInit = {
                        ...
                        purchaseReceiptSwitchWarehouseUrl: '',
                ...
                <div
                    ...
                    @keydown.escape.window="purchaseReceiptEscape()"
                    @scroll.window="repositionOpenSuggests()"
                    @resize.window="repositionOpenSuggests()"
                    x-data="purchaseReceiptForm()"
                    class="pr-create-page grid min-h-0 w-full min-w-0 max-w-full grid-cols-1 items-stretch gap-5"
                >
                    {{-- Левая колонка: та же высота, что у «Корзины» --}}
                    <div class="relative z-20 flex h-full min-h-0 min-w-0 flex-col overflow-visible">
                        <div class="pr-panel-shell flex h-full min-h-0 min-w-0 flex-col overflow-visible border border-teal-800/12 bg-white shadow-[0_4px_24px_-8px_rgba(15,23,42,0.12)] ring-1 ring-teal-900/[0.05]">
                            <div class="pr-panel-header-teal shrink-0">
                                <label for="pr_header_good_q">Наименование товара</label>
                            </div>
                            <div class="relative flex min-h-0 flex-1 flex-col bg-white px-4 pb-5 pt-4 sm:px-[1.125rem] sm:pb-6 sm:pt-5" x-ref="headerGoodBlock">
                                        ...
                                        >
                                            ...
                                            >
                                                <button
                                                    ...
                                                    @click.prevent.stop="openNewGoodModal(headerGoodQuery)"
                                                >
                ...
                >
                    ...
                    <div class="pr-cart-card flex min-h-0 flex-1 flex-col overflow-hidden border bg-white ring-1 ring-teal-900/[0.05]">
                            ...
                            >
                                <thead>
                                    <tr>
                                        ...
                                        <th class="min-w-[8rem]">Наименование *</th>
                                        <th>Наценка %</th>
                                        <th>Кол-во *</th>
                                        <th>Цена (закуп.) *</th>
                                        <th>Сумма</th>
                                    ...
                                    <template x-for="(row, index) in lines" :key="index">
                                        ...
                                        >
                                            ...
                                            <td class="min-w-[8rem]">
                                                ...
                                                <input type="hidden" :name="`lines[${index}][unit]`" x-model="row.unit" autocomplete="off" />
                                                <input
                                                    type="text"
                                            ...
                                            <td class="min-w-[5.75rem] ob-numr">
                                                <input
                                                    type="text"
                                            ...
                                            <td class="min-w-[6.25rem] ob-sum ob-numr">
                                                <input type="text" class="ob-inp" readonly tabindex="-1" :value="lineSum(row)" />
                                            </td>
                                            <td class="min-w-[6.75rem] ob-numr">
                                                <input
                                                    type="text"
                                                    ...
                                                    inputmode="decimal"
                                                    autocomplete="off"
                                                    title="При ручном изменении пересчитывается наценка % от закупочной цены"
                                                    @input="applyMarkupFromSale(index)"
                                                />
                                            </td>
                        ...
                        <div class="ob-1c-foot shrink-0">
                            <button type="button" class="ob-tb-btn ob-btn-submit pr-checkout-btn inline-flex items-center gap-1.5 px-3 py-1.5 !text-xs" @click="openFinalizeModal()">
                ...
                <template x-teleport="body">
                    <div
                        ...
                        role="dialog"
                        aria-modal="true"
                        aria-labelledby="pr-finalize-title"
                    >
                            ...
                            >
                                <div class="bg-[#008b8b] px-5 py-4 text-white">
                                    <h3 id="pr-finalize-title" class="text-base font-bold tracking-tight">Сохранение поступления</h3>
                                    <p class="mt-1.5 text-sm font-normal leading-snug text-white/85">
                                        Поставщик, дата и склад документа — затем сохраните изменения.
                                    </p>
                                </div>
                                <div class="space-y-5 px-5 py-5">
                                    <div>
                                        <span class="mb-0 block text-sm font-semibold text-slate-700">Склад документа</span>
                                        <p class="mt-2 rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 text-sm font-medium text-slate-900">{{ $purchaseReceipt->warehouse?->name ?? '—' }}</p>
                                        <p class="mt-1 text-xs text-slate-500">Склад после проведения не меняется.</p>
                                    </div>
                                    <div class="grid grid-cols-1 gap-5 sm:grid-cols-2">
                                        <div class="relative min-w-0" x-ref="supplierRoot">
                                            <label for="pr_finalize_supplier" class="mb-0 block text-sm font-semibold text-slate-700">Поставщик</label>
                                            <input
                                                id="pr_finalize_supplier"
                                                type="text"
                                                name="supplier_name"
                                                form="pr-purchase-receipt-form"
                                        ...
                                        <div class="min-w-0">
                                            <label for="pr_finalize_document_date" class="mb-0 block text-sm font-semibold text-slate-700">Дата документа *</label>
                                            <input
                                                id="pr_finalize_document_date"
                                                type="date"
                                                name="document_date"
                                                form="pr-purchase-receipt-form"
                                ...
                                <div class="flex shrink-0 flex-wrap justify-end gap-2.5 border-t border-slate-100 bg-slate-50/70 px-5 py-4">
                                    ...
                                    <button
                                        ...
                                        @click.prevent.stop="submitPurchaseFromModal()"
                                    >
                                        Сохранить изменения
                                    </button>
                                </div>
                ...
                @include('admin.partials.good-quick-create-modal', ['idPrefix' => 'pr'])
                    </div>
                </div>
        @endif
    </div>
