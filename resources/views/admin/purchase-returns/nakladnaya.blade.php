@php
    ...
    $documentTitle = InvoiceNakladnayaFormatter::purchaseReturnDocumentTitle($purchaseReturn->document_date, $purchaseReturn->id);
...
<body>
    @if (! ($forPdf ?? false))
        <div class="no-print">
            ...
            <span style="margin-left:8px;font-size:10pt;color:#64748b;">PDF: в диалоге печати выберите «Сохранить как PDF» или кнопку ниже.</span>
        </div>
        @if (isset($printOrganizations) && $printOrganizations->count() > 1)
            @php
                $printUrl = route('admin.purchase-returns.print', $purchaseReturn);
                $pdfUrl = route('admin.purchase-returns.pdf', $purchaseReturn);
    ...
    <div class="meta-block">
        ...
        <div class="meta-row">
            ...
            <span class="val">{{ $purchaseReturn->warehouse->name }}</span>
        </div>
        <div class="meta-row">
            <span class="lbl">Поставщик:</span>
            <span class="val">{{ $purchaseReturn->supplier_name !== '' ? $purchaseReturn->supplier_name : '—' }}</span>
        </div>
    </div>
