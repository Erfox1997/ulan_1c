<x-admin-layout pageTitle="Возврат поставщику" main-class="px-3 py-5 sm:px-4 lg:px-5">
        ...
        @else
            <div
                ...
                x-data="{
                    ...
                    prtBase: @js(rtrim(url('/admin/p/trade.purchase-return'), '/')),
            ...
            >
                <div class="rounded-xl border border-slate-200/90 bg-white px-4 py-4 shadow-sm ring-1 ring-slate-900/5 sm:px-6">
                    <div class="flex w-full min-w-0 flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                        <div class="flex min-w-0 flex-wrap items-center gap-x-4 gap-y-2">
                            <h2 class="text-lg font-semibold tracking-tight text-slate-900">Возврат поставщику</h2>
                            @if ($selectedWarehouseId !== 0)
                                <a
                                    href="{{ route('admin.purchase-returns.create', ['warehouse_id' => $selectedWarehouseId]) }}"
                        ...
                        <form
                            method="GET"
                            action="{{ route('admin.purchase-returns.index') }}"
                        ...
                        >
                            <div class="flex w-full min-w-0 items-center gap-3 rounded-lg border border-slate-200 bg-slate-50/90 px-3 py-1.5 sm:inline-flex sm:w-auto sm:py-2">
                                <label for="prt_warehouse" class="shrink-0 text-xs font-medium text-slate-600">Склад</label>
                                <select
                                    id="prt_warehouse"
                                    name="warehouse_id"
                ...
                <div class="mt-10 overflow-hidden rounded-xl border border-slate-200/90 bg-white shadow-sm ring-1 ring-slate-900/5">
                    <div class="flex flex-wrap items-center justify-between gap-3 border-b border-slate-200 bg-slate-50/80 px-5 py-3.5 sm:px-6">
                        <h3 class="text-sm font-semibold tracking-tight text-slate-900">Журнал возвратов</h3>
                        @if (! $recentReturns->isEmpty())
                            <div class="flex flex-wrap items-center gap-2 sm:gap-3">
                                @if ($organizations->isNotEmpty())
                                    <div class="flex w-full min-w-0 flex-wrap items-center gap-2 sm:w-auto">
                                        <label for="prt_print_org" class="shrink-0 text-[11px] font-medium text-slate-600">Организация в шапке</label>
                                        <select
                                            id="prt_print_org"
                                            x-model="printOrgId"
                                ...
                                <button
                                    ...
                                    @click="selectedReturnId && window.open(prtBase + '/' + selectedReturnId + '/print' + (printOrgId != null && printOrgId !== '' ? '?organization_id=' + printOrgId : ''), '_blank')"
                                >
                                    Печать накладной
                                </button>
                                <button
                                    ...
                                    class="inline-flex items-center rounded-md border border-slate-300 bg-white px-3 py-1.5 text-xs font-medium text-slate-700 shadow-sm transition hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-50"
                                    :disabled="!selectedReturnId"
                                    @click="selectedReturnId && (window.location.href = prtBase + '/' + selectedReturnId + '/pdf' + (printOrgId != null && printOrgId !== '' ? '?organization_id=' + printOrgId : ''))"
                                >
                                    Скачать PDF
                                </button>
                            </div>
                                ...
                                <tbody class="bg-white">
                                    @foreach ($recentReturns as $r)
                                        <tr
                                            ...
                                            role="link"
                                            tabindex="0"
                                            data-edit-url="{{ route('admin.purchase-returns.edit', $r) }}"
                                            onclick="if (!event.target.closest('td[data-prt-sel]') && !event.target.closest('td[data-doc-action]')) { window.location.href=this.dataset.editUrl }"
                                            onkeydown="if(event.key==='Enter'||event.key===' '){event.preventDefault();window.location.href=this.dataset.editUrl}"
                                        >
                                            <td data-prt-sel class="border border-slate-300 px-2 py-2 text-center" @click.stop>
                                                <input
                                                    type="radio"
                                                    name="prt_journal_pick"
                                                    value="{{ $r->id }}"
                                                    class="h-4 w-4 cursor-pointer border-slate-300 text-emerald-600 focus:ring-emerald-500"
                                                    x-model="selectedReturnId"
                                                />
                                            </td>
                                            ...
                                            <td data-doc-action class="border border-slate-300 px-2 py-2 text-center" @click.stop>
                                                <form
                                                    method="POST"
                                                    action="{{ route('admin.purchase-returns.destroy', $r) }}"
                                                    class="inline"
                                                    onsubmit="return confirm('Удалить возврат поставщику от {{ $r->document_date->format('d.m.Y') }}? Остатки будут пересчитаны.');"
                                                >
                                                    @csrf
                                                    @method('DELETE')
