<x-admin-layout pageTitle="Редактирование реализации покупателю (юрлицо)" main-class="px-3 py-6 sm:px-6 lg:px-8">
                    ...
                    <div class="overflow-hidden rounded-[1.65rem] bg-gradient-to-b from-white/95 to-slate-50/90">
                        <div class="ob-1c-scope overflow-hidden rounded-[1.5rem] bg-white/95">
                            ...
                            >
                                ...
                                <h2 class="text-[15px] font-bold leading-tight text-slate-800">Редактирование реализации юридическому лицу</h2>
                            </div>
                            ...
                            <div class="ob-1c-header">
                                <div class="les-header-primary-row flex w-full min-w-0 flex-wrap items-end gap-x-4 gap-y-3">
                                    ...
                                    <div class="les-date-doc-wrap shrink-0">
                                        <label for="document_date">Дата документа *</label>
                                        <input
                                            id="document_date"
                                            type="date"
                                            name="document_date"
                                            value="{{ old('document_date', $legalEntitySale->document_date->format('Y-m-d')) }}"
                                            required
                                        />
                                    </div>
                            ...
                            <div class="ob-1c-toolbar">
                                ...
                                @if ($organizations->isNotEmpty())
                                    <span
                                        class="inline-flex max-w-full flex-wrap items-center gap-1.5"
                                        x-data="{ printOrgId: {{ $defaultPrintOrganizationId !== null ? (int) $defaultPrintOrganizationId : 'null' }} }"
                                    >
                                        <label class="shrink-0 text-[10px] font-semibold text-slate-600" title="Поле «Организация» на накладной">Орг.</label>
                                        <select
                                            x-model="printOrgId"
                                            class="max-w-[10rem] shrink rounded-lg border border-slate-200/90 bg-white px-1.5 py-0.5 text-[10px] font-medium text-slate-800 shadow-sm ring-1 ring-slate-900/5"
                                        >
                                        ...
                                        <a
                                            :href="`{{ route('admin.legal-entity-sales.work-order-print', $legalEntitySale) }}${printOrgId != null ? '?organization_id=' + printOrgId : ''}`"
                                            target="_blank"
                                            rel="noopener noreferrer"
                                            class="ob-tb-btn !no-underline"
                                        >Заказ-наряд</a>
                                        <a
                                            :href="`{{ route('admin.legal-entity-sales.print', $legalEntitySale) }}${printOrgId != null ? '?organization_id=' + printOrgId : ''}`"
                                            target="_blank"
                                            rel="noopener noreferrer"
                                            class="ob-tb-btn !no-underline"
                                        >Печать накладной</a>
                                        <a
                                            :href="`{{ route('admin.legal-entity-sales.pdf', $legalEntitySale) }}${printOrgId != null ? '?organization_id=' + printOrgId : ''}`"
                                            class="ob-tb-btn !no-underline"
                                        >Скачать PDF</a>
                                    </span>
                                @else
                                    <a
                                        href="{{ route('admin.legal-entity-sales.work-order-print', $legalEntitySale) }}"
                                        target="_blank"
                                        rel="noopener noreferrer"
                                        class="ob-tb-btn !no-underline"
                                    >Заказ-наряд</a>
                                    <a
                                        href="{{ route('admin.legal-entity-sales.print', $legalEntitySale) }}"
                                        target="_blank"
                                        rel="noopener noreferrer"
                                        class="ob-tb-btn !no-underline"
                                    >Печать накладной</a>
                                    <a
                                        href="{{ route('admin.legal-entity-sales.pdf', $legalEntitySale) }}"
                                        class="ob-tb-btn !no-underline"
                                    >Скачать PDF</a>
                                @endif
                                <span class="mx-1 h-4 w-px bg-slate-300/90" aria-hidden="true"></span>
                            ...
                            <div
                                ...
                                class="fixed z-[200] max-h-56 overflow-y-auto rounded-xl border border-slate-200/90 bg-white py-1 text-left shadow-[0_12px_40px_-8px_rgba(15,23,42,0.18)] ring-1 ring-slate-900/5"
                                role="listbox"
