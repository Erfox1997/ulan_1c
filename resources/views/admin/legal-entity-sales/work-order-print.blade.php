@php
    ...
    $buyerName = trim((string) ($les->buyer_name ?? ''));
    $buyerLabel = $buyerName !== '' ? $buyerName : '—';
    $buyerPin = trim((string) ($les->buyer_pin ?? ''));
    $buyerPinDisp = $buyerPin !== '' ? $buyerPin : '—';
    $whName = $les->warehouse?->name ?? '—';
    $comment = $les->comment !== null && trim((string) $les->comment) !== '' ? trim((string) $les->comment) : '—';
    $docDateStr = $les->document_date?->format('d.m.Y') ?? $les->created_at?->format('d.m.Y') ?? '—';
...
<body>
    ...
    @if (isset($printOrganizations) && $printOrganizations->count() > 1)
        @php
            $woPrintUrl = route('admin.legal-entity-sales.work-order-print', $legalEntitySale);
        @endphp
        <div class="no-print" style="margin:0 0 12px;padding:8px 10px;border:1px solid #cbd5e1;border-radius:6px;background:#f8fafc;font-size:10pt;">
            <label for="les_wo_org" style="margin-right:8px;">Организация в шапке</label>
            <select
                id="les_wo_org"
                onchange="var v=this.value||'';var q=v?'?organization_id='+v:'';location.href='{{ $woPrintUrl }}'+q"
                style="min-width:14rem;padding:4px 8px;border:1px solid #cbd5e1;border-radius:4px;"
            >
    ...
    @else
        <table class="meta" aria-label="Реквизиты реализации">
            <tr>
                <td class="lbl">Дата документа</td>
                <td>{{ $docDateStr }}</td>
                <td class="lbl">Покупатель</td>
                <td>{{ $buyerLabel }}</td>
            </tr>
            <tr>
                <td class="lbl">ИНН / ПИН</td>
                <td style="font-family: DejaVu Sans Mono, monospace;">{{ $buyerPinDisp }}</td>
                <td class="lbl">Склад</td>
                <td>{{ $whName }}</td>
            </tr>
            <tr>
                <td class="lbl">Примечание</td>
    ...
    <div class="signatures">
        ...
        @else
            <div class="row">Отпустил ______________________</div>
            <div class="row">Получил ______________________ {{ $buyerLabel }}</div>
        @endif
    </div>
</body>
</html>
