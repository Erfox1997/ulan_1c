<x-admin-layout :pageTitle="$pageTitle" main-class="bg-slate-100/80 px-3 py-4 sm:px-4 lg:px-6">
    <div class="mx-auto max-w-5xl space-y-4">
        ...
        <div class="flex flex-wrap items-center justify-between gap-3">
            <form method="GET" action="{{ route('admin.stock.incoming') }}" class="flex flex-wrap items-end gap-3">
                <div>
            ...
            <a href="{{ route('admin.stock.incoming.create', ['warehouse_id' => $selectedWarehouseId]) }}" class="inline-flex rounded-lg bg-indigo-600 px-4 py-2 text-sm font-semibold text-white shadow-sm hover:bg-indigo-700">Новое оприходование</a>
        </div>
        ...
        @else
            <div class="overflow-hidden rounded-xl border border-slate-200/90 bg-white shadow-sm">
                <div class="border-b border-slate-100 px-4 py-3">
                    <h2 class="text-sm font-bold text-slate-900">Журнал оприходований</h2>
                    <p class="text-xs text-slate-500">Излишки и прочее поступление без поставщика. Нажмите на строку, чтобы открыть и изменить документ.</p>
                </div>
                    ...
                    <table class="min-w-full text-left text-sm">
                        ...
                        <tbody class="divide-y divide-slate-100">
                            @forelse ($documents as $d)
                                @php $editUrl = route('admin.stock.incoming.edit', $d); @endphp
