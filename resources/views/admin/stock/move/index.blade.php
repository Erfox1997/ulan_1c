<x-admin-layout :pageTitle="$pageTitle" main-class="bg-slate-100/80 px-3 py-4 sm:px-4 lg:px-6">
    <div class="mx-auto max-w-5xl space-y-4">
        @if (session('status'))
            <div class="rounded-xl border border-indigo-200/80 bg-indigo-50 px-4 py-3 text-sm text-indigo-900">
                {{ session('status') }}
            </div>
        @endif

        <div class="flex flex-wrap items-center justify-between gap-3">
            <form method="GET" action="{{ route('admin.stock.move') }}" class="flex flex-wrap items-end gap-3">
                <div>
                    <label for="st_mv_wh" class="mb-1 block text-xs font-semibold text-slate-700">Склад в журнале</label>
                    <select
                        id="st_mv_wh"
                        name="warehouse_id"
            ...
            <a
                href="{{ route('admin.stock.move.create', ['from_warehouse_id' => $selectedWarehouseId]) }}"
        ...
        @else
            <div class="overflow-hidden rounded-xl border border-slate-200/90 bg-white shadow-sm">
                <div class="border-b border-slate-100 px-4 py-3 sm:px-5">
                    <h2 class="text-sm font-bold text-slate-900">Журнал перемещений</h2>
                    <p class="text-xs text-slate-500">Списание со склада-отправителя и приход на склад-получателя по средней закупочной цене.</p>
                </div>
                    ...
                    <table class="min-w-full text-left text-sm">
                        <thead class="border-b border-slate-200 bg-slate-50 text-[10px] font-bold uppercase text-slate-500">
                            <tr>
                                ...
                                <th class="px-4 py-2">Откуда → куда</th>
                        ...
                        <tbody class="divide-y divide-slate-100">
                            @forelse ($documents as $d)
                                <tr class="transition-colors hover:bg-indigo-50/60">
                                    <td class="whitespace-nowrap px-4 py-2.5 text-slate-800">{{ $d->document_date->format('d.m.Y') }}</td>
                            ...
                            @empty
                                <tr>
                                    <td colspan="3" class="px-4 py-10 text-center text-sm text-slate-500">Документов пока нет.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
