    ...
    <div class="mx-auto max-w-6xl space-y-4">
        <div class="overflow-hidden rounded-xl border border-slate-200/90 bg-white shadow-md ring-1 ring-slate-900/[0.04]">
            ...
            >
                ...
                <p class="mt-0.5 text-[11px] font-medium text-emerald-100/90">Текущие количества по складу (начальные остатки, закупки, продажи, перемещения и др.)</p>
            </div>

            <div class="border-b border-slate-100 px-4 py-3 sm:px-5">
                <form method="GET" action="{{ route('admin.stock.report-balances') }}" class="flex flex-wrap items-end gap-3">
                    <div class="min-w-[12rem] flex-1">
                        <label for="stock_bal_wh" class="mb-1 block text-xs font-semibold text-slate-700">Склад</label>
                        <select
                            id="stock_bal_wh"
                            name="warehouse_id"
                        ...
                        <tbody class="divide-y divide-slate-100">
                            @forelse ($rows as $b)
                                <tr class="hover:bg-emerald-50/30">
                                    <td class="whitespace-nowrap px-4 py-2.5 font-mono text-xs text-slate-600">{{ $b->good?->article_code ?? '—' }}</td>
                                    <td class="px-4 py-2.5 font-medium text-slate-900">{{ $b->good?->name ?? '—' }}</td>
                                    <td class="whitespace-nowrap px-4 py-2.5 text-slate-600">{{ $b->good?->unit ?? 'шт.' }}</td>
                                    <td class="whitespace-nowrap px-4 py-2.5 text-right font-semibold tabular-nums text-slate-900">{{ $fmtQty($b->quantity) }}</td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="4" class="px-4 py-12 text-center text-sm text-slate-500">
                                        Нет строк остатков на выбранном складе. Заполните <a href="{{ route('admin.opening-balances.index') }}" class="font-semibold text-emerald-700 underline">начальные остатки</a>.
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
