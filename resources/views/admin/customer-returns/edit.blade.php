<x-admin-layout pageTitle="Редактирование возврата от клиента" main-class="px-3 py-5 sm:px-4 lg:px-5">
    <div class="w-full min-w-0 space-y-6">
        ...
        @else
            <div class="rounded-xl border border-slate-200/90 bg-white px-5 py-4 shadow-sm ring-1 ring-slate-900/5">
                <p class="text-[11px] font-semibold uppercase tracking-wide text-slate-500">Склад документа</p>
                <p class="mt-1 text-sm font-medium text-slate-900">{{ $customerReturn->warehouse->name }}</p>
            ...
            <script>
                window.__customerReturnInit = {
                    ...
                    buyerName: @json(old('buyer_name', $customerReturn->buyer_name)),
                    warehouseId: {{ (int) $selectedWarehouseId }},
                    branchName: @json(auth()->user()->branch?->name ?? ''),
                    warehouseName: @json($customerReturn->warehouse->name ?? ''),
                };
            </script>
            <form
                method="POST"
                action="{{ route('admin.customer-returns.update', $customerReturn) }}"
                class="space-y-6"
            ...
            >
                ...
                <div class="ob-1c-scope rounded-sm border border-[#c0c0c0] bg-white p-0 shadow-sm">
                    <div class="border-b border-[#c0c0c0] bg-[#f5f5f5] px-3 py-2">
                        <h2 class="text-sm font-semibold text-neutral-800">Редактирование возврата от клиента</h2>
                        <p class="mt-0.5 text-[11px] text-neutral-600">Пересохранение пересчитывает остатки на складе.</p>
                    </div>
                    ...
                    <div class="ob-1c-header">
                        <div class="relative" x-ref="buyerRoot">
                            ...
                            >
                                ...
                                >
                                    ...
                                    <label class="block text-[10px] font-semibold text-slate-600" for="cr_edit_cp_quick_legal_form">Правовая форма</label>
                                    <select
                                        id="cr_edit_cp_quick_legal_form"
                        ...
                        <div>
                            ...
                            <input
                                ...
                                type="date"
                                name="document_date"
                                value="{{ old('document_date', $customerReturn->document_date->format('Y-m-d')) }}"
                                required
                            />
                        </div>
                    </div>
                    ...
                    <div class="ob-1c-toolbar">
                        ...
                        @if ($organizations->isNotEmpty())
                            ...
                            >
                                ...
                                <a
                                    :href="`{{ route('admin.customer-returns.print', $customerReturn) }}${printOrgId != null ? '?organization_id=' + printOrgId : ''}`"
                                    target="_blank"
                                ...
                                >Печать накладной</a>
                                <a
                                    :href="`{{ route('admin.customer-returns.pdf', $customerReturn) }}${printOrgId != null ? '?organization_id=' + printOrgId : ''}`"
                                    class="ob-tb-btn"
                                    style="text-decoration:none;color:inherit;"
                                >Скачать PDF</a>
                            </span>
                        @else
                            <a
                                href="{{ route('admin.customer-returns.print', $customerReturn) }}"
                                target="_blank"
                                rel="noopener noreferrer"
                                class="ob-tb-btn"
                                style="text-decoration:none;color:inherit;"
                            >Печать накладной</a>
                            <a
                                href="{{ route('admin.customer-returns.pdf', $customerReturn) }}"
                                class="ob-tb-btn"
                                style="text-decoration:none;color:inherit;"
                            >Скачать PDF</a>
                        @endif
