<x-admin-layout pageTitle="Возврат от покупателя" main-class="px-3 py-5 sm:px-4 lg:px-5">
    <div class="mx-auto flex w-full max-w-6xl flex-col gap-10">
        ...
        @else
            <div
                ...
                x-data="{
                    selectedId: null,
                    crBase: @js(rtrim(url('/admin/p/trade.return-client'), '/')),
                ...
                <div class="rounded-xl border border-slate-200/90 bg-white px-4 py-4 shadow-sm ring-1 ring-slate-900/5 sm:px-6">
                    <div class="flex w-full min-w-0 flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                        <div class="flex min-w-0 flex-wrap items-center gap-x-4 gap-y-2">
                            <h2 class="text-lg font-semibold tracking-tight text-slate-900">Возврат товаров от клиента</h2>
                            @if ($selectedWarehouseId !== 0)
                                <a
                                    href="{{ route('admin.customer-returns.create', ['warehouse_id' => $selectedWarehouseId]) }}"
                        ...
                        <form
                            method="GET"
                            action="{{ route('admin.customer-returns.index') }}"
                        ...
                        >
                            <div class="flex w-full min-w-0 items-center gap-3 rounded-lg border border-slate-200 bg-slate-50/90 px-3 py-1.5 sm:inline-flex sm:w-auto sm:py-2">
                                <label for="cr_warehouse" class="shrink-0 text-xs font-medium text-slate-600">Склад</label>
                                <select
                                    id="cr_warehouse"
                                    name="warehouse_id"
                            ...
                            <div class="flex flex-wrap items-center gap-2 sm:gap-3">
                                @if ($organizations->isNotEmpty())
                                    <div class="flex w-full min-w-0 flex-wrap items-center gap-2 sm:w-auto">
                                        <label for="cr_print_org" class="shrink-0 text-[11px] font-medium text-slate-600">Организация в шапке</label>
                                        <select
                                            id="cr_print_org"
                                            x-model="printOrgId"
                                ...
                                <button
                                    ...
                                    :disabled="!selectedId"
                                    @click="selectedId && window.open(crBase + '/' + selectedId + '/print' + (printOrgId != null && printOrgId !== '' ? '?organization_id=' + printOrgId : ''), '_blank')"
                                >
                                    Печать накладной
                                </button>
                                <button
                                    ...
                                    class="inline-flex items-center rounded-md border border-slate-300 bg-white px-3 py-1.5 text-xs font-medium text-slate-700 shadow-sm transition hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-50"
                                    :disabled="!selectedId"
                                    @click="selectedId && (window.location.href = crBase + '/' + selectedId + '/pdf' + (printOrgId != null && printOrgId !== '' ? '?organization_id=' + printOrgId : ''))"
                                >
                                    Скачать PDF
                                </button>
                            </div>
                            ...
                            <table class="w-full border-collapse border border-slate-300 text-sm">
                                <thead>
                                    <tr class="bg-slate-100">
                                        ...
                                        <th scope="col" class="border border-slate-300 px-3 py-2.5 text-left text-[11px] font-semibold uppercase tracking-wide text-slate-700">Клиент</th>
                                ...
                                <tbody class="bg-white">
                                    @foreach ($recentReturns as $r)
                                        <tr
                                            ...
                                            role="link"
                                            tabindex="0"
                                            data-edit-url="{{ route('admin.customer-returns.edit', $r) }}"
                                            onclick="if (!event.target.closest('td[data-cr-sel]') && !event.target.closest('td[data-doc-action]')) { window.location.href=this.dataset.editUrl }"
                                            onkeydown="if(event.key==='Enter'||event.key===' '){event.preventDefault();window.location.href=this.dataset.editUrl}"
                                        >
                                            <td data-cr-sel class="border border-slate-300 px-2 py-2 text-center" @click.stop>
                                                <input
                                                    type="radio"
                                                    name="cr_journal_pick"
                                                    value="{{ $r->id }}"
                                            ...
                                            <td data-doc-action class="border border-slate-300 px-2 py-2 text-center" @click.stop>
                                                <form
                                                    method="POST"
                                                    action="{{ route('admin.customer-returns.destroy', $r) }}"
                                                    class="inline"
                                                    onsubmit="return confirm('Удалить возврат от покупателя от {{ $r->document_date->format('d.m.Y') }}? Остатки будут пересчитаны.');"
                                                >
                                                    @csrf
                                                    @method('DELETE')
