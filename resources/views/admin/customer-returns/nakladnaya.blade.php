@php
    ...
    $documentTitle = InvoiceNakladnayaFormatter::customerReturnDocumentTitle($customerReturn->document_date, $customerReturn->id);
...
<body>
    @if (! ($forPdf ?? false))
        ...
        @if (isset($printOrganizations) && $printOrganizations->count() > 1)
            @php
                $printUrl = route('admin.customer-returns.print', $customerReturn);
                $pdfUrl = route('admin.customer-returns.pdf', $customerReturn);
    ...
    <div class="meta-block">
        ...
        <div class="meta-row">
            ...
            <span class="val">{{ $customerReturn->warehouse->name }}</span>
        </div>
        <div class="meta-row">
            <span class="lbl">Клиент:</span>
            <span class="val">{{ $customerReturn->buyer_name !== '' ? $customerReturn->buyer_name : '—' }}</span>
        </div>
    </div>
    ...
    <div class="signatures">
        <div class="sign-left">Принял (на склад) _______________________</div>
        <div class="sign-right">Сдал (клиент) _______________________</div>
    </div>
</body>
</html>
