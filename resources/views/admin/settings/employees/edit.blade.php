        ...
        <div class="overflow-hidden rounded-xl border border-slate-200/90 bg-white shadow-md ring-1 ring-slate-900/[0.04]">
            ...
            <form method="POST" action="{{ route('admin.settings.employees.update', $employee) }}" class="space-y-5 px-4 py-5 sm:px-6">
                ...
                <div>
                    ...
                    >
                        <option value="" disabled @selected(old('job_type', $employee->job_type) === null || old('job_type', $employee->job_type) === '')>Выберите</option>
                        @foreach (\App\Models\Employee::JOB_TYPE_LABELS as $key => $label)
                            <option value="{{ $key }}" @selected(old('job_type', $employee->job_type) === $key)>{{ $label }}</option>
                        @endforeach
                    </select>
                </div>
