    ...
    <div class="mx-auto max-w-2xl space-y-6">
        ...
        <div class="overflow-hidden rounded-xl border border-slate-200/90 bg-white shadow-md ring-1 ring-slate-900/[0.04]">
            ...
            >
                ...
                <p class="mt-0.5 text-[11px] font-medium text-emerald-100/90">Создаётся учётная запись для входа на сайт (логин = e-mail).</p>
            </div>

            <form method="POST" action="{{ route('admin.settings.employees.store') }}" class="space-y-5 px-4 py-5 sm:px-6">
                @csrf
                ...
                <div class="grid gap-4 sm:grid-cols-2">
                    <div>
                        <label class="mb-1 block text-xs font-semibold text-slate-700">Пароль *</label>
                        <input
                            id="password"
                            type="password"
                            name="password"
                ...
                <div>
                    ...
                    >
                        <option value="">— Полный доступ (как у главного администратора) —</option>
                        @foreach ($roles as $role)
                            <option value="{{ $role->id }}" @selected((string) old('branch_role_id') === (string) $role->id)>{{ $role->name }}{{ $role->is_full_access ? ' (полный доступ)' : '' }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="flex flex-wrap gap-3">
                    ...
                    <a href="{{ route('admin.settings.employees') }}" class="rounded-lg border border-slate-200 bg-white px-4 py-2 text-sm font-medium text-slate-800 hover:bg-slate-50">Отмена</a>
                </div>
            </form>
        </div>
    </div>
